...
var tty = require('tty');
var require$$1$3 = require('supports-color');
...
function inflate(stream, options) {
  ...
  return stream.pipe(zlib__default['default'].Unzip(options))
}
...
var json = async function(req, opts) {
  req = req.req || req;
  ...
  const str = await rawBody(inflation(req), opts);
  try {
...
var form = async function(req, opts) {
  req = req.req || req;
  opts = utils.clone(opts);
...
var text = async function(req, opts) {
  req = req.req || req;
  opts = utils.clone(opts);
...
var coBody = createCommonjsModule(function (module, exports) {
...
var koaBodyparser = function (opts) {
  ...
  async function parseBody(ctx) {
    if (enableJson && ((detectJSON && detectJSON(ctx)) || ctx.request.is(jsonTypes))) {
      return await coBody.json(ctx, jsonOpts);
    }
    if (enableForm && ctx.request.is(formTypes)) {
      return await coBody.form(ctx, formOpts);
    }
    if (enableText && ctx.request.is(textTypes)) {
      return await coBody.text(ctx, textOpts) || '';
    }
    if (enableXml && ctx.request.is(xmlTypes)) {
      return await coBody.text(ctx, xmlOpts) || '';
    }
    return {};
  }
};
...
try {
  var supportsColor = require$$1__default$2['default'];
...
var Writable = require$$1__default$1['default'].Writable;
...
var AddSubscription = /*#__PURE__*/Object.freeze({
	__proto__: null,
	path: path,
	method: method,
	callback: callback
...
const router = new Router__default['default']({ prefix: "/.netlify/functions/server/subscription" });
...
async function main$1(ctx, next) {
    await next();
}
